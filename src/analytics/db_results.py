    ...
    try:
        ...
        for row in rows:
            ...
            if matches:
                ...
                # вебхук (оплата на сайте) и лишь ПОТОМ организатор вручную
                # разобрал/проставил номера в Excel — если не помечать их
                # source='import' при апдейте, get_age_group_label() для них
                # так и остаётся на "Ошибка возраста" (source не менялся с
                # DEFAULT 'webhook'), хотя организатор их уже вручную
                # проверил в этом самом файле (реальный вопрос пользователя,
                # 2026-08-19: "я импортировал файл... всё ещё вижу «Ошибка
