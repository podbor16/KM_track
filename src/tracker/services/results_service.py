...
# Singleflight: один поток пересчитывает, остальные ждут и берут из кеша
_build_locks: dict = {}
...
def build_event_results(
    ...
    # Singleflight: получить/создать лок для этого ключа
    ...
    # Только один поток строит результат; остальные ждут на локе и берут из кеша
    with _key_lock:
        _now_resp2 = time.time()
        if _resp_key in _response_cache and (_now_resp2 - _response_cache_ts.get(_resp_key, 0)) < RESPONSE_CACHE_TTL:
            return _response_cache[_resp_key]
