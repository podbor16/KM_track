...
from src.krasmarafon.models import (
...
from src.krasmarafon.services import get_formatted_analytics
from src.krasmarafon import services as _svc
...
async def get_registered_runners(
    ...
    try:
        ...
        from src.krasmarafon.models.analytics import RegisteredRunnerInfo
...
async def get_event_results(
    ...
    import time
    import asyncio
    from src.krasmarafon.services.results_service import (
...
async def fetch_race_data_now():
    ...
    from src.krasmarafon.parsers import fetch_data, save_to_file
    try:
        data = fetch_data()
        if data is not None:
...
async def get_startlist(event_id: int = PathParam(..., description="ID события в БД")):
    ...
    from src.krasmarafon.models.startlist import StartlistItem, StartlistResponse
