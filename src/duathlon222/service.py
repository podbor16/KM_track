...
# Дистанции этапов целиком — из stage_fields (финиш), не из кругов. Бег-2 —
# 42.2 (не ровно 42): реальная измеренная дистанция по кругам сходится к
# ~42.19 км (25×1.757 + offset, см. LAP_KM/STAGE_LAP_OFFSET ниже) — то же
# число, что уже показывалось как последняя отметка ("42.19"), просто раньше
# не совпадало с этой заявленной "круглой" константой (найдено пользователем
# 2026-09-05, на реальной гонке).
STAGE_KM = {"run1": 10.0, "bike": 170.0, "run2": 42.2}
