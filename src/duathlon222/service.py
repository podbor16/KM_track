...
# подогнано на глаз:
#   Вело: от старта (0 км) до первой мачты — 3.4 км (короткий "пролог"), не
#     полные 4.04 км; ВСЕ последующие отметки — уже ровно по 4.04 км друг от
#     друга. distance(n) = n*4.04 + (3.4-4.04) даёт ровно 3.4 при n=1 и
#     ровно 169.04 при n=42 (последняя реальная отметка — "b169,04"; сам
#     финиш вело, 170 км, отдельное поле "b170-start_T2", НЕ входит в этот
#     список кругов вовсе — 0.96 км между последней отметкой и финишем это
#     не баг, так реально измерена трасса).
#   Бег-2: отметки НЕ точно "n*1.757" — реальные поля идут 1777, 3534, ...,
#     42188 метров, т.е. n*1757 + 20 (фиксированные +20 м на КАЖДОЙ отметке,
#     не только первой) — найдено 2026-09-04 по карточке участника
#     (последняя отметка показывала "42.17" вместо реальных "42.19").
...
STAGE_LAP_OFFSET = {"run1": 0.0, "bike": 3.4 - 4.04, "run2": 0.02}
