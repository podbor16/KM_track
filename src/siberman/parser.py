...
CHECKPOINT_COL_MAP: dict[tuple[str, int], tuple[str, int]] = {
    # Плавание: 4 круга по 2,5 км, с буем-разворотом на середине каждого
    # круга. Префикс "Плавание: " обязателен — иначе "N круг(а)" совпадает
    # (через startswith-эвристику) с одноимёнными заголовками бега ниже
    # ("1 круг" — префикс "1 круг плавания", коллизия occurrence-индекса).
    ("плавание: разворот 1", 0):  ("swim", 1),
    ("плавание: 1 круг", 0):      ("swim", 2),
    ("плавание: разворот 2", 0):  ("swim", 3),
    ("плавание: 2 круга", 0):     ("swim", 4),
    ("плавание: разворот 3", 0):  ("swim", 5),
    ("плавание: 3 круга", 0):     ("swim", 6),
    ...
    ("196.7 км", 0):          ("bike_day2", 5),   # "190 км Кольцо Саяногорск" — startswith
    ("205.6 км", 0):          ("bike_day2", 6),
...
# Ни одна КТ на дистанциях Siberman физически не может занять меньше минуты.
# Реальный кейс: судья вводит "0:35,01" (имея в виду 0ч 35мин 01с) — САМ Excel
# (русская локаль) распознаёт это как валидное время формата М:СС,сс (0 мин
# 35.01 сек) ДО того, как файл сохраняется, и наш код получает уже готовое
# число 35 секунд — без единой строки-мусора, которую можно было бы поймать
# при парсинге. Отследить можно только по итоговому неправдоподобному значению.
MIN_PLAUSIBLE_TIME_S = 60
...
def _classify_time_cell(value) -> tuple[Optional[int], Optional[str]]:
    ...
    отлавливает неправдоподобные значения (опечатка разделителя, в т.ч. те,
    что Excel успел молча "исправить" сам ещё до сохранения файла)."""
    seconds = parse_time_to_seconds(value)
    if seconds is not None:
        if seconds > MAX_PLAUSIBLE_TIME_S:
            return None, (
                f"неправдоподобно большое время «{value}» → {seconds} с "
                f"(проверьте разделитель — должен быть Ч:ММ:СС через двоеточие, не точку)"
            )
        if seconds < MIN_PLAUSIBLE_TIME_S:
            return None, (
                f"неправдоподобно маленькое время «{value}» → {seconds} с "
                f"(меньше минуты — проверьте ячейку, возможно перепутан разделитель)"
            )
        return seconds, None
    if value is None:
        return None, None
