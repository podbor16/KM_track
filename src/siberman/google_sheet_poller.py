"""
⚠ FALLBACK-скрипт (2026-08-06): основной способ синхронизации теперь —
встроенный лидер-цикл в app.py (lifespan, _siberman_google_sheet_poller),
...
(двойная синхронизация: не критично благодаря upsert-семантике, но лишняя
нагрузка).
...
следующего интервала). Запуск:

    python -m src.siberman.google_sheet_poller --race-year 2026 --interval 20

Останов — Ctrl+C.

⚠ Пока синхронизация включена (в админке ИЛИ этот процесс активен) — НЕ
запускать обычную Excel-загрузку в админке для этого года (apply_to_db()
делает clear_race_year() и сотрёт результат последнего цикла синка).
"""
import argparse
import logging
import time
