...
2026-08-06: основной вызывающий код — встроенный лидер-цикл в app.py
(lifespan, _siberman_google_sheet_poller), который крутится всегда во
всех воркерах и сам проверяет race_config.google_sheet_sync_enabled на
каждой итерации — "Включить"/"Выключить" в админке буквально
запускают/останавливают опрос, без SSH. google_sheet_poller.py (отдельный
CLI-процесс) остаётся как fallback для локальной отладки без Redis.
"""
import hashlib
import logging
import re
