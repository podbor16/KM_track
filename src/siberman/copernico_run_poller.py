"""
⚠ FALLBACK-скрипт (2026-08-06): основной способ опроса Copernico теперь —
встроенный лидер-цикл в app.py (lifespan, _siberman_copernico_run_poller),
включаемый/выключаемый прямо в админке без SSH. Этот CLI-скрипт нужен
только для локальной отладки без Redis (в DEBUG-режиме встроенный цикл не
запускается) — НЕ запускать его одновременно с продовым приложением
(двойной опрос: не критично благодаря upsert-семантике, но лишняя
нагрузка на Copernico/БД).

По образцу load_race_results.py::continuous_mode (тот же backoff-
паттерн). Запуск:
...
⚠ Пока запись включена (в админке ИЛИ этот процесс активен) для бегового
этапа — НЕ запускать обычный Excel apply (для любого этапа): apply_to_db()
делает clear_race_year() и сотрёт всё, что уже собрано по бегу.
"""
import argparse
import logging
import time
