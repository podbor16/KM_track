...
const CHECKPOINT_LABELS = {
    ...
    bike_day2: {
        1: '51 км', 2: '83 км', 3: '119 км', 4: '160 км (СШГЭС)',
        5: '196.7 км (Кольцо Саяногорск)', 6: '205.6 км', 7: '265 км',
        8: 'Финиш (276 км)',
    },
...
const CHECKPOINT_DIST_KM = {
    ...
    bike_day2: { 1: 51, 2: 83, 3: 119, 4: 160, 5: 196.7, 6: 205.6, 7: 265, 8: 276 },
...
// Прогноз времени до ЛЮБОЙ будущей точки (следующая КТ / финиш этапа) —
// линейная экстраполяция средней скорости С НАЧАЛА ЭТАПА. Единая формула
// для всех дисциплин (плавание/вело/бег) — не нужно km/ч или темп/100м как
// промежуточный шаг, это просто пропорция расстояний. Используется и в
// results.html (колонка "Прогноз финиша"), и в participant.html (колонка
// "Общее время" для ещё не пройденных КТ) — единственное место с формулой
// (см. docs/superpowers/specs/2026-08-06-siberman-finish-forecast-design.md).
// НЕ кэшируется нигде — пересчитывается на каждый рендер из текущего pos,
// поэтому пересчёт при достижении новой КТ происходит "сам собой".
function forecastTime(distSoFarKm, elapsedS, targetDistKm) {
    if (!(distSoFarKm > 0) || elapsedS == null) return null;
    // Округляем до целых секунд — fmtTime() не округляет дробную часть
    // секунд сама (там `s % 60` без Math.floor для секундной части),
    // нецелый elapsedS дал бы в UI что-то вроде "4:40:52.222222".
