...
def _fetch_copernico(race_id: str, login: str, preset: str, event: str) -> list:
    ...
    with urllib.request.urlopen(req, timeout=20) as resp:
        ...
        # запроса на это — сырые байты тогда начинаются с magic-number
        # \x1f\x8b, и .decode("utf-8") падает с "invalid start byte" (тот же
        # баг найден и исправлен в load_duathlon_results.py, 2026-09-05).
        if raw[:2] == b"\x1f\x8b":
