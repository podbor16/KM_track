...
1. **Одна таблица для всего:** Все данные хранятся в одной таблице `Все заявки`
...
# Инициализация (с настройками по умолчанию для таблицы Все заявки)
...
- В `.env` указаны правильные настройки подключения к БД:
  ...
  DB_PORT=your_port
  ```
- База данных доступна и содержит таблицу `Все заявки`
...
4. **Кириллица в названии таблицы:** Таблица называется `Все заявки` (с кириллицей), поэтому в запросах используются обратные кавычки.
